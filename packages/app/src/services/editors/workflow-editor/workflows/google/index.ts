import {
  ClapWorkflow,
  ClapWorkflowEngine,
  ClapWorkflowCategory,
  ClapWorkflowProvider,
} from '@aitube/clap'

import {
  genericHeight1024,
  genericHeight2048,
  genericImage,
  genericImageUrl,
  genericPrompt,
  genericVideo,
  genericWidth1024,
  genericWidth2048,
} from '../common/defaultValues'

// ------------------------------------------------------------------------------
// if a user is already using one of those workflows and you change its settings,
// they will have to reselect it in the UI for changes to be taken into account.
//
// -> we can create a ticket to fix this
// ------------------------------------------------------------------------------

// you can find the list here: https://cloud.google.com/vertex-ai/generative-ai/docs/learn/model-versioning
// The auto-updated version of a Gemini model points to the most recent stable version
// google supports claude so you can also see the ids specific to Vertex AI here:
// https://docs.anthropic.com/en/docs/models-overview
export const googleWorkflows: ClapWorkflow[] = [
  {
    id: 'google://gemini-3.0-pro',
    label: 'Gemini 3.0 Pro',
    description: '',
    tags: ['Gemini', 'pro', '3.0'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.ASSISTANT,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'gemini-3.0-pro',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://gemini-2.0-flash-001',
    label: 'Gemini 2.0 Flash (001)',
    description: '',
    tags: ['Gemini', 'flash', '2.0'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.ASSISTANT,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'gemini-2.0-flash-001',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://gemini-1.5-flash-001',
    label: 'Gemini 1.5 flash (001)',
    description: '',
    tags: ['Gemini', 'flash'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.ASSISTANT,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'gemini-1.5-flash-001',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://gemini-1.5-pro-001',
    label: 'Gemini 1.5 pro (001)',
    description: '',
    tags: ['Gemini', 'pro'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.ASSISTANT,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'gemini-1.5-pro-001',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://gemini-1.0-vision-001',
    label: 'Gemini 1.0 vision (001)',
    description: '',
    tags: ['Gemini', 'vision'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.ASSISTANT,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'gemini-1.0-vision-001',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://gemini-1.0-pro-002',
    label: 'Gemini 1.0 pro (002)',
    description: '',
    tags: ['Gemini', 'vision'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.ASSISTANT,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'gemini-1.0-pro-002',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://claude-3.5-20240620',
    label: 'Claude 3.5 (Sonnet)',
    description: '',
    tags: ['Claude', 'Sonnet'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.ASSISTANT,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'claude-3-5-sonnet@20240620',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://claude-3-opus@20240229',
    label: 'Claude 3 (Opus)',
    description: '',
    tags: ['Claude', 'Opus'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.ASSISTANT,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'claude-3-opus@20240229',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://claude-3-haiku@20240307',
    label: 'Claude 3 (Haiku)',
    description: '',
    tags: ['Claude', 'Haiku'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.ASSISTANT,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'claude-3-haiku@20240307',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  // Video Generation Models
  {
    id: 'google://veo-2',
    label: 'Veo 2',
    description: 'Google Veo 2 video generation model',
    tags: ['Veo', 'video', '2'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.VIDEO_GENERATION,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'veo-2',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://veo-3',
    label: 'Veo 3',
    description: 'Google Veo 3 video generation model',
    tags: ['Veo', 'video', '3'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.VIDEO_GENERATION,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'veo-3',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://veo-3.1',
    label: 'Veo 3.1',
    description: 'Google Veo 3.1 video generation model',
    tags: ['Veo', 'video', '3.1'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.VIDEO_GENERATION,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'veo-3.1',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://imagegeneration@006',
    label: 'Imagen 2 (006)',
    description: 'Google Imagen 2 image generation model (stable, widely available)',
    tags: ['Imagen', 'image', '2', '006'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.IMAGE_GENERATION,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'imagegeneration@006',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://imagen-3.0-generate-001',
    label: 'Imagen 3 (generate-001)',
    description: 'Google Imagen 3 image generation model (stable, GA)',
    tags: ['Imagen', 'image', '3'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.IMAGE_GENERATION,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'imagen-3.0-generate-001',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://imagen-3.0-fast-generate-001',
    label: 'Imagen 3 Fast (generate-001)',
    description: 'Google Imagen 3 Fast image generation model (stable, GA)',
    tags: ['Imagen', 'image', '3', 'fast'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.IMAGE_GENERATION,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'imagen-3.0-fast-generate-001',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://imagen-4',
    label: 'Imagen 4',
    description: 'Google Imagen 4 image generation model',
    tags: ['Imagen', 'image', '4'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.IMAGE_GENERATION,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'imagen-4',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://imagen-4-flash',
    label: 'Imagen 4 Flash',
    description: 'Google Imagen 4 Flash image generation model (fast)',
    tags: ['Imagen', 'image', '4', 'flash'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.IMAGE_GENERATION,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'imagen-4-flash',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://imagen-4-pro',
    label: 'Imagen 4 Pro',
    description: 'Google Imagen 4 Pro image generation model',
    tags: ['Imagen', 'image', '4', 'pro'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.IMAGE_GENERATION,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'imagen-4-pro',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  {
    id: 'google://imagen-4-turbo',
    label: 'Imagen 4 Turbo',
    description: 'Google Imagen 4 Turbo image generation model',
    tags: ['Imagen', 'image', '4', 'turbo'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.IMAGE_GENERATION,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'imagen-4-turbo',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
  // Music Generation Models
  {
    id: 'google://lyria-2',
    label: 'Lyria 2',
    description: 'Google Lyria 2 music generation model',
    tags: ['Lyria', 'music', '2'],
    author: 'Google (GCP Vertex AI)',
    thumbnailUrl: '',
    nonCommercial: false,
    engine: ClapWorkflowEngine.REST_API,
    category: ClapWorkflowCategory.MUSIC_GENERATION,
    provider: ClapWorkflowProvider.GOOGLE,
    data: 'lyria-2',
    schema: '',
    inputFields: [genericPrompt],
    inputValues: {
      prompt: genericPrompt.defaultValue,
    },
  },
]
